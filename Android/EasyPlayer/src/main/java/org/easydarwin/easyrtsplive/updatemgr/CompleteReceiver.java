package org.easydarwin.easyrtsplive.updatemgr;
...
public class CompleteReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(DownloadManager.ACTION_DOWNLOAD_COMPLETE)) {
			...
			Log.d(TAG, "kim columnCount ="+columnCount);
			...
			while(cursor.moveToNext()) {
				for (int j = 0; j < columnCount; j++) {
					...
					Log.d(TAG, "kim cursor ="+cursor);

					if(columnName.equals("local_filename")&&string!=null&&string.endsWith("apk")) {
						Log.d(TAG, "kim installIntent string="+string);
